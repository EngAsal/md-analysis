...
hbond_mon = open_hbond_file('monomer-details.dat')
...
hbond_all.fillna({'chainA': 0, 'chainB': 0, 'chainC': 0, 'monomer':0}, inplace=True)
...
filtered_df = final_df[~((final_df['ABC_stdev'] == 0) & (final_df['monomer'] <= 5))]
#%%
pivot_df = filtered_df.pivot(index='Donor', columns='Acceptor', values='diff')
pivot_mon = hbond_mon.pivot(index='Donor', columns='Acceptor', values='monomer')
...
max_row = filtered_df.loc[max_index_A]

min_index_A = filtered_df['diff'].idxmin()
min_row = filtered_df.loc[min_index_A]

print("row with max diff is", max_row)
print("row with min diff is", min_row)
#%%
...
# %%
### H-bond analysis with MDA's tools instead of dat file from VMD 
os.chdir('/home/pghw87/Documents/md-sim/5ue6/monomer/monomer')
u = mda.Universe("protein.tpr","final_center_aligned_res49_362_sliced_100.xtc")
#%%
# Define selections (modify based on your system)
donors = "protein"
acceptors = "protein"

# Perform Hydrogen Bond Analysis
hba = HBA(u, donors_sel=donors, acceptors_sel=acceptors)
hba.run()

# Extract hbond data: [frame, donor_id, hydrogen_id, acceptor_id, distance, angle]
hbonds = hba.hbonds

# Total number of frames
total_frames = len(u.trajectory)

# Dictionary to store bond counts
hbond_counts = defaultdict(int)

# Process hbonds and count occurrences
for frame, donor, hydrogen, acceptor, _, _ in hbonds:
    bond_pair = (int(donor), int(acceptor))  # Identify bond by donor-acceptor pair
    hbond_counts[bond_pair] += 1

# Create DataFrame
df = pd.DataFrame(
    [(bond[0], bond[1], (count / total_frames) * 100) for bond, count in hbond_counts.items()],
    columns=["Donor ID", "Acceptor ID", "Occupancy (%)"]
)
...
# df.to_csv("hbonds_occupancy.csv", index=False)
# %%
